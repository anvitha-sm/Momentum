...
router.get("/saved/:userId", async (req, res) => {
    try {
        const user = req.user; 
        const savedWorkouts = await Workout.find({ _id: { $in: user.savedWorkouts } });
...
router.post("/create", async (req, res) => {
    try {
        ...
        const user = req.user
        const newWorkout = new Workout({ name, description, movements, createdUser: user._id });
...
router.post("/save/:workoutId", async (req, res) => {
    try {

...
import { logWorkout, getAllLoggedWorkoutsAPI, addToScheduleAPI, getAllUserWorkoutsAPI } from "../api/api";
        ...
        {workoutsToLog.length > 0 ? (
            workoutsToLog.map((workout) => (
              <div className="workout-card">
                <img src={workout.imageUrl} alt={workout.className} className="workout-image">
                </img>
                <div className="workout-details">
                  <p className="workout-name"> {workout.name}</p>
                  <p className="workout-duration">Duration: {workout.duration} min </p>
                  <p className="workout-intensity">Intensity: {workout.intensity}</p>
                </div>
              </div>
            ))
          ) :(
            <div className="no-workouts-message">
              ...
              <button className="log-workout" onClick={navigate("/dashboard")}> Add New Workouts </button>
            </div>
          )
        }
      </div>
